...
/* Export a function that logs in |user| by creating 
 * an authToken and sending it back
 * as a cookie, creating a Session for |user|, and saving 
 * it in |sessions| indexed
 * by the authToken.

 * 1 Cookie is tagged by |cookieName|, times out on the 
 * client side after |duration|
 * (though the router, below, will check anyway to prevent 
 * hacking), and will not be
...
/* Export a router that will find any Session associated with 
 * |req|, based on cookies,
 * delete the Session if it has timed out, or attach the Session 
 * to |req| if it's current
 * If |req| has an attached Session after this process, 
 * then down-chain routes will
